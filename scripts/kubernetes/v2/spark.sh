...
options=(
    "spark{-py|-r}:v2.4.6 Docker base images, build and push" \
    "spark-py:v2.4.6-plus-{branch} Docker image, build and push" \
    "SparkPi example" \
...
select opt in "${options[@]}"; do
    case $opt in
        "spark{-py|-r}:v2.4.6 Docker base images, build and push")
            ...
            bash ./scripts/kubernetes/v2/base.sh
            break
            ;;
        ...
        "spark-py:v2.4.6-plus-{branch} Docker image, build and push")
            ...
            bash ./scripts/kubernetes/v2/python/plus.sh
            break
            ;;

        "SparkPi example")
            K8S_MASTER=$(kubectl cluster-info | sed -r "s/\x1B\[([0-9]{1,3}(;[0-9]{1,2})?)?[mGK]//g" | awk -F' ' 'NR==1{print $6}')
            read -p 'Docker image [borjagilperez/spark-py:v2.4.6-plus-latest]: ' IMAGE
            if [ -z "$IMAGE" ]; then
                IMAGE='borjagilperez/spark-py:v2.4.6-plus-latest'
            fi
